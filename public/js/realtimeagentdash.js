var RealTime = {
	init: function (result) {
		...
		if (!ran) {
			...
			for (var i = 0; i < result.length; i++) {
				...
				this.load_array(status_type, status_arrays, result[i][1]);
			}
		} else {
			// process each returned list
			for (var i = 0; i < result.length; i++) {
				let status_type = result[i][0];
				this.process_array(status_type, status_arrays, result[i][1]);
			}
		}
	...
	load_array(list_id, array_data, result_data) {
		...
		for (var i = 0; i < result_data.length; i++) {
			array_data[list_id].push({ 'Login': result_data[i]['Login'], 'checksum': result_data[i]['checksum'] });
	...
	process_array(list_id, array_data, result_data) {
		...
		if (result_data.length == 0) {
			if (array_data[list_id].length > 0) {
				this.delete_all_rows(list_id);
				array_data[list_id] = [];
			}
		} else {
			...
			let delete_list = [];
			for (var i = 0; i < array_data[list_id].length; i++) {
				// look for login in results
				let row_obj = result_data.find(row => row.Login === array_data[list_id][i].Login);
				if (row_obj) {
					if (row_obj.checksum != array_data[list_id][i].checksum) {
						array_data[list_id][i].checksum = row_obj.checksum;
						this.update_row(list_id, row_obj);
					}
				} else {
					delete_list.push(array_data[list_id][i].Login);
				}
			}

			// now delete the ones that need it
			for (var i = 0; i < delete_list.length; i++) {
				array_data[list_id] = array_data[list_id].filter(row => row.Login != delete_list[i]);
				this.delete_row(list_id, delete_list[i]);
			}

			// now go thru the returned list and insert if necc
			for (var i = 0; i < result_data.length; i++) {
				if (!array_data[list_id].find(row => row.Login === result_data[i]['Login'])) {
					array_data[list_id].push({ 'Login': result_data[i]['Login'], 'checksum': result_data[i]['checksum'] });
					this.add_row(list_id, result_data[i]);
				}
			}

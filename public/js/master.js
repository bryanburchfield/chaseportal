var Master = {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                for(var i=0;i<obj.length;i++){
                    campaign_searchresults+='<li class><a href="#">'+obj[i]+'</a></li>';
                }

                Master.first_search=false;

                $('.filter_campaign').append(campaign_searchresults);
                $('.filter_campaign li').each(function(){
                    if($(this).find('a').text() == Master.active_camp_search){
    ...
    select_database:function(e){
        ...
        if(checked){
            ...
            if(type != 'report'){
                Dashboard.set_databases(databases);
            }else{
                $.ajax({
                    url: 'set_database',
                    type: 'POST',
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(response.status != 'success'){
                    ...
                    $('.users table tbody').append('<tr id="user'+response.user_id+'" data-id="'+response.user_id+'"><td>'+group_id+' - '+ name+'</td><td><a href="admin.php?action=edit&user_id='+response.user_id+'" class="edit_user"><i class="fas fa-user-edit"></i></a></td><td><a data-toggle="modal" data-target="#deleteUserModal" class="remove_user" href="#" data-name="'+name+'" data-user="'+response.user_id+'"><i class="glyphicon glyphicon-remove-sign"></i></a></td>');
                    setTimeout(function(){ 
                        $('.alert').remove();

@if(!session()->has('isSso'))
    <div class="btn-group d-flex float-right">
         <button type="button" class="btn btn-default bl_none" onclick="window.location.href = '{{ action('AutomatedReportController@automatedReports') }}';">
            <span>{{__('general.auto_reports')}}</span>
        </button>
    </div>
@elseif(session()->has('isSsoSuperadmin'))
    <div class="btn-group d-flex float-right col-sm-4 p0">
    ...
    <div class="btn-group d-flex float-right col-sm-4">
        <div class="form-group sso">
